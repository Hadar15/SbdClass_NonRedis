...
const getRemainingStock = async (db: Queryable, eventId: string) => {
  const eventResult = await db.query('SELECT total_stock FROM events WHERE id = $1', [eventId]);
  if (eventResult.rowCount === 0) throw new Error('Event not found');

  const paidResult = await db.query(
    'SELECT COUNT(*)::int AS count FROM transactions WHERE event_id = $1 AND status = $2',
    [eventId, 'PAID']
  );
  const pendingResult = await db.query(
    'SELECT COUNT(*)::int AS count FROM transactions WHERE event_id = $1 AND status = $2 AND expires_at > now()',
    [eventId, 'PENDING']
  );
  ...
  const result = await withTransaction(async (client) => {
    if (process.env.STRICT_QUEUE === 'true') {
      const promotionResult = await client.query(
        'SELECT status, promotion_expires_at FROM queue_entries WHERE event_id = $1 AND user_id = $2',
        [eventId, userId]
      );
    ...
    const existingResult = await client.query(
      'SELECT id FROM transactions WHERE event_id = $1 AND user_id = $2 AND (status = $3 OR (status = $4 AND expires_at > now())) LIMIT 1',
      [eventId, userId, 'PAID', 'PENDING']
    );
    ...
    const insertResult = await client.query(
      'INSERT INTO transactions (user_id, event_id, status, expires_at, created_at, updated_at) VALUES ($1, $2, $3, $4, now(), now()) RETURNING id, expires_at',
      [userId, eventId, 'PENDING', expiresAt]
    );

    await client.query(
      'UPDATE queue_entries SET status = $1, promotion_expires_at = NULL, updated_at = now() WHERE event_id = $2 AND user_id = $3 AND status = $4',
      ['DONE', eventId, userId, 'PROMOTED']
    );

    return { expiresAt: insertResult.rows[0]?.expires_at as Date };
  });
...
export const confirmPayment = async (eventId: string, userId: string) => {
  const transaction = await withTransaction(async (client) => {
    const existingResult = await client.query(
      'SELECT id FROM transactions WHERE event_id = $1 AND user_id = $2 AND status = $3 AND expires_at > now() ORDER BY created_at DESC LIMIT 1',
...
export const getTicketStatus = async (eventId: string, userId: string) => {
  const existingResult = await query(
    'SELECT expires_at FROM transactions WHERE event_id = $1 AND user_id = $2 AND status = $3 AND expires_at > now() ORDER BY created_at DESC LIMIT 1',
    [eventId, userId, 'PENDING']
  );

  if (existingResult.rowCount === 0) return { status: 'none' };

  const expiresAt = existingResult.rows[0]?.expires_at as Date;
...
export const expireReservations = async () => {
  await query(
    'UPDATE transactions SET status = $1, updated_at = now() WHERE status = $2 AND expires_at < now()',
    ['EXPIRED', 'PENDING']
  );
};

export const getEventStock = async (eventId: string) => {
